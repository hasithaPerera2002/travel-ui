...
$(document).ready(async function () {
  ...
  let packages = await fetch(
    "http://localhost:8081/api/v1/package/all",
    requestOptions
  );
...
$("#addPackageButton").click(function (event) {
  ...
  if (validateForm()) {
    ...
    fetch("http://localhost:8000/api/v1/package/save", requestOptions)
...
function validateForm() {
  ...
  const file = $('#addPackageForm input[type="file"][name="image"]')[0]
    .files[0];

  if (url.trim() === "") {
    $('#addPackageForm input[name="videoUrl"]')
      .next(".error-message")
  ...
  if (voted.trim() === "" || isNaN(voted) || parseFloat(voted) < 0) {
    $('#addPackageForm input[name="votedCount"]')
      .next(".error-message")
      .text("Voted count is not valid");
  ...
  if (
    ...
    $('#addPackageForm input[name="stars"]')
      .next(".error-message")
      .text("Stars is not valid");
  ...
  if (!file) {
    $('#addPackageForm input[type="file"][name="image"]')
      .next(".error-message")
...
function deletePackage(id) {
  ...
  $("#confirmDelete").on("click", function () {
    ...
    fetch(
      `http://localhost:8000/api/v1/package/delete?id=${id}`,
      requestOptions
    )
      .then(function (response) {
        if (response.ok) {
...
const validateUpdateForm = () => {
  ...
  const file = $('#updatePackageForm input[type="file"][name="image"]')[0]
    .files[0];

  if (url.trim() === "") {
    $('#updatePackageForm input[name="videoUrl"]')
      .next(".error-message")
      .text("Video url cannot be empty");
  ...
  if (voted.trim() === "" || isNaN(voted) || parseFloat(voted) < 0) {
    $('#updatePackageForm input[name="votedCount"]')
      .next(".error-message")
      .text("Voted count is not valid");
  ...
  if (
    ...
    $('#updatePackageForm input[name="stars"]')
      .next(".error-message")
      .text("Stars is not valid");
  ...
  if (!file) {
    $('#updatePackageForm input[type="file"][name="image"]')
      .next(".error-message")
      .text("Image is required");
...
function updatePackage(id) {
  ...
  $("#updatePackage").click(function (event) {
    ...
    if (validateUpdateForm()) {
      ...
      formdata.append(
        "videoUrl",
        $('#updatePackageForm input[name="videoUrl"]').val()
      );
      formdata.append("placeName", $("#updatePlaces").val());
      formdata.append("packageType", $("#updateCategory").val());
      formdata.append(
        "noOfPeople",
        $('#updatePackageForm input[name="votedCount"]').val()
      );
      formdata.append(
        "stars",
        $('#updatePackageForm input[name="stars"]').val()
      );
      formdata.append(
        "placeImage",
        $('#updatePackageForm input[type="file"][name="image"]')[0].files[0]
      );
      ...
      fetch(
        `http://localhost:8000/api/v1/package/update?id=${id}`,
        requestOptions
      )
        .then(function (response) {
          if (response.ok) {
