...
$(document).ready(async function () {
  ...
  let guides = await fetch(`http://localhost:8000/api/v1/guide/all`, {
    method: "GET",
    headers: {},
    cors: "no-cors",
  });
...
$("#addGuideButton").click(function (event) {
  if (validateForm()) {
    ...
    formdata.append(
      "guideContact",
      $('#guideForm input[name="contact"]').val()
    );
    ...
    formdata.append(
      "val_per_day",
      $('#guideForm input[name="ValuePerDay"]').val()
    );
    ...
    fetch("http://localhost:8000/api/v1/guide/save", requestOptions)
...
function validateForm() {
  ...
  var guideIdFront = $(
    '#guideForm input[type="file"][name="GuideIdFront"]'
  ).val();
  var guideIdBack = $(
    '#guideForm input[type="file"][name="GuideIdBack"]'
  ).val();
  var guideNicFront = $(
    '#guideForm input[type="file"][name="GuideNicFront"]'
  ).val();
  var guideNicBack = $(
    '#guideForm input[type="file"][name="GuideNicBack"]'
  ).val();
  var valuePerDay = $(
    '#guideForm input[type="number"][name="ValuePerDay"]'
  ).val();
  ...
  if (isNaN(valuePerDay) || parseFloat(valuePerDay) <= 0) {
    $("#addValuePerDay")
      .next(".error-message")
      .text("Value per day is not valid");
  ...
  if (!guideIdFront) {
    $("#addGuideIdFront")
      .next(".error-message")
      .text("Guide ID front image is required");
  ...
  if (!guideIdBack) {
    $("#addGuideIdBack")
      .next(".error-message")
      .text("Guide ID back image is required");
  ...
  if (!guideNicFront) {
    $("#addGuideNicFront")
      .next(".error-message")
      .text("Guide NIC front image is required");
  ...
  if (!guideNicBack) {
    $("#addGuideNicBack")
      .next(".error-message")
      .text("Guide NIC back image is required");
...
function deleteGuide(id) {
  ...
  $("#confirmDelete").on("click", function () {
    ...
    fetch(`http://localhost:8000/api/v1/guide/delete?id=${id}`, requestOptions)
      .then(function (response) {
        if (response.ok) {
...
const validateUpdateForm = () => {
  ...
  var guideImage = $(
    '#updateGuideForm input[type="file"][name="GuideImage"]'
  ).val();
  var guideIdFront = $(
    '#updateGuideForm input[type="file"][name="GuideIdFront"]'
  ).val();
  var guideIdBack = $(
    '#updateGuideForm input[type="file"][name="GuideIdBack"]'
  ).val();
  var guideNicFront = $(
    '#updateGuideForm input[type="file"][name="GuideNicFront"]'
  ).val();
  var guideNicBack = $(
    '#updateGuideForm input[type="file"][name="GuideNicBack"]'
  ).val();
  var valuePerDay = $(
    '#updateGuideForm input[type="number"][name="ValuePerDay"]'
  ).val();
  ...
  if (name.trim() === "") {
    $('#updateGuideForm input[name="name"]')
      .next(".error-message")
  ...
  if (contact.trim() === "") {
    $('#updateGuideForm input[type="tel"]')
      .next(".error-message")
  ...
  if (parseFloat(age) <= 0 || isNaN(age)) {
    $('#updateGuideForm input[name="age"]')
      .next(".error-message")
  ...
  if (!validateEmail(email)) {
    $('#updateGuideForm input[type="email"]')
      .next(".error-message")
...
function updateGuide(id) {
  ...
  $("#updateGuide").click(function (event) {
    if (validateUpdateForm()) {
      ...
      formdata.append(
        "guideName",
        $('#updateGuideForm input[name="name"]').val()
      );
      formdata.append("guideAddress", $("#updateAddress").val());
      formdata.append(
        "guideContact",
        $('#updateGuideForm input[name="contact"]').val()
      );
      formdata.append(
        "guideEmail",
        $('#updateGuideForm input[type="email"]').val()
      );
      formdata.append(
        "guideAge",
        $('#updateGuideForm input[name="age"]').val()
      );
      formdata.append("guideGender", "MALE");
      formdata.append(
        "val_per_day",
        $('#updateGuideForm input[name="ValuePerDay"]').val()
      );
      formdata.append(
        "isAvailable",
        $('#updateGuideForm input[name="available"]').val()
      );
      formdata.append(
        "experience",
        $('#updateGuideForm input[name="exp"]').val()
      );
      formdata.append(
        "guideImage",
        $('#updateGuideForm input[name="GuideImage"]')[0].files[0]
      );
      formdata.append(
        "nic_front_image",
        $('#updateGuideForm input[name="GuideNicFront"]')[0].files[0]
      );
      formdata.append(
        "nic_back_image",
        $('#updateGuideForm input[name="GuideNicBack"]')[0].files[0]
      );
      formdata.append(
        "guide_id_front_image",
        $('#updateGuideForm input[name="GuideIdFront"]')[0].files[0]
      );
      formdata.append(
        "guide_id_back_image",
        $('#updateGuideForm input[name="GuideIdBack"]')[0].files[0]
      );
      ...
      fetch(
        `http://localhost:8000/api/v1/guide/update?id=${id}`,
        requestOptions
      )
        .then(function (response) {
          if (response.ok) {
