...
$(document).ready(async function () {
    ...
    let users = await fetch("http://localhost:8080/api/v1/user/all", requestOptions);
...
$('#addUserButton').click(function (event) {
    ...
    if (validateForm()) {
        ...
        formdata.append("firstName",$('#addUserForm input[name="firstName"]').val());
        formdata.append("secondName", $('#addUserForm input[name="secondName"]').val());
        ...
        formdata.append("profilePicture", $('#addUserForm input[name="profilePicture"]')[0].files[0]);
        formdata.append("nicNumber", $('#addUserForm input[name="nicNumber"]').val());
        formdata.append("backImageOfNIC",$('input[name="nicBackImage"]')[0].files[0]);
        formdata.append("frontImageOfNIC",$('input[name="nicFrontImage"]')[0].files[0]);
...
function validateForm() {
    ...
    if (firstName === null || firstName === undefined || firstName === '' || (!/^[a-zA-Z0-9_-]{3,16}$/.test(firstName))) {
        $('#addUserForm input[name="firstName"]').next('.error-message').text('first name  invalid');
    ...
    if (secondName === null || secondName === undefined || secondName === '' ||(!/^[a-zA-Z0-9_-]{3,16}$/.test(secondName))) {
        $('#addUserForm input[name="secondName"]').next('.error-message').text('second name  invalid');
        return false;
    }else {
    ...
    if(!validateEmail(email)){
        $('#addUserForm input[type="email"]').next('.error-message').text('email is invalid');
    ...
    if (contactNumber.length > 20) {
        $('#addUserForm input[type="tel"]').next('.error-message').text('contact number  cannot be empty');
    ...
    if(!profilePic){
        $('#addUserForm input[name="profilePicture"]').next('.error-message').text('profile picture is required');
        $('#addUserForm input[name="profilePicture"]').focus();
        return false;
    }else {
        $('#addUserForm input[name="profilePicture"]').next('.error-message').text('');
    }

    if(!nicFrontImage){
        $('#addUserForm input[name="nicFrontImage"]').next('.error-message').text('nic front image is required');
        $('#addUserForm input[name="nicFrontImage"]').focus();
        return false;
    }else {
        $('#addUserForm input[name="nicFrontImage"]').next('.error-message').text('');
    }

    if(!nicBackImage){
        $('#addUserForm input[name="nicBackImage"]').next('.error-message').text('nic back image is required');
        $('#addUserForm input[name="nicBackImage"]').focus();
        return false;
    }else {
        $('#addUserForm input[name="nicBackImage"]').next('.error-message').text('');
    }

    if (nicNumber === null || nicNumber === undefined || nicNumber === '' || nicNumber.length < 10) {
        $('#addUserForm input[name="nicNumber"]').next('.error-message').text('nic number  cannot be empty and >10');
...
function deleteUser(id) {
    ...
    $('#confirmDelete').on('click', function () {
        ...
        fetch(`http://localhost:8080/api/v1/user/delete?id=${id}`, requestOptions)
            .then(function (response) {
                if (response.ok) {
